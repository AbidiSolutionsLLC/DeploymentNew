...
const TableWithPagination = ({
 ...
 <thead>
 <tr className="bg-surface text-heading">
 ...
 className={`p-2 rounded-xl transition-all shadow-sm ${typeof action.className === 'function' ? action.className(row) : (action.className || 'bg-surface text-muted hover:bg-surface hover:text-brand hover:shadow-md')}`}
 title={action.title}
 >
 {action.icon}
 </button>
 ...
 <p className="text-sm font-bold text-muted uppercase tracking-widest">{emptyMessage}</p>
 </div>
 </td>
 </tr>
 )}
 ...
 className="p-2 rounded-xl bg-surface text-main hover:bg-surface hover:shadow-md disabled:opacity-40 disabled:cursor-not-allowed transition-all"
 title="First Page"
 >
 ...
 <div className="flex items-center gap-2">
 <span className="text-[10px] uppercase tracking-widest font-black text-muted">Rows per page:</span>
 <select
