  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-[10px] font-black text-muted dark:text-slate-500 mb-2 uppercase tracking-widest">
            SUMMARY DESCRIPTION* <span className="normal-case font-normal text-muted dark:text-slate-500">(min 10, max 500 chars)</span>
          </label>
          <textarea
            value={description}
            onChange={(e) => {
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <p className="text-[10px] text-muted dark:text-slate-400 text-right">{description.length}/500</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative group">
            ...
            />
            <div className="w-full bg-surface/50 dark:bg-slate-800/50 border border-border-subtle dark:border-slate-700 border-dashed rounded-xl px-4 py-3 text-sm text-muted dark:text-slate-400 flex items-center justify-between group-hover:bg-surface dark:group-hover:bg-slate-800 transition-all">
              <span className="truncate">
                {attachments.length > 0 ? `${attachments.length} file(s) selected` : "Choose file(s)..."}
              </span>
              <span className="text-[10px] font-black text-muted dark:text-slate-400 uppercase">UPLOAD</span>
            </div>
          </div>
          {attachmentError && (
              ...
              {attachments.map((file, index) => (
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span className="text-[9px] text-muted dark:text-slate-400">({(file.size / 1024).toFixed(1)} KB)</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => removeAttachment(index)}
                    className="text-muted dark:text-slate-400 hover:text-red-500 transition-colors text-sm font-bold"
                  >
                    ✕
                  </button>
                </div>
