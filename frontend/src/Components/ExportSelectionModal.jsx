    ...
    <GlassModal
      ...
      title={
        <div className="flex flex-col">
          <span>{title}</span>
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-0.5 uppercase tracking-wide">
      ...
      <div className="flex flex-col h-full space-y-4">
        ...
        <div className="space-y-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted dark:text-slate-400" size={16} />
            <input 
              type="text" 
          ...
          <div className="flex items-center justify-between px-2">
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <span className="text-xs font-bold text-muted dark:text-slate-400 uppercase tracking-wide">Select All Visible</span>
            </label>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto max-h-[50vh] pr-2 custom-scrollbar-visible">
          {filteredItems.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <p className="text-[11px] font-bold text-muted dark:text-slate-400 uppercase">
                        {item.date ? new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : "N/A"}
                      </p>
                    </div>
                    <div className="flex justify-between items-center mt-0.5">
                      <p className="text-xs text-muted dark:text-slate-400 font-medium">{item.name || "Unnamed Timesheet"}</p>
            ...
            <div className="py-12 text-center">
              <p className="text-sm font-medium text-muted dark:text-slate-400">No items match your search</p>
            </div>
          )}
        </div>
      </div>
