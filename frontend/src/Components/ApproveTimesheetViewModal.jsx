      ...
      title={
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-center justify-center gap-3 mt-2">
            <p className="text-sm font-medium text-muted dark:text-slate-400 lowercase capitalize">{timesheet.name}</p>
      ...
      <div className="space-y-6">
        ...
        {!isApprovedTab && timesheet.status === "Pending" && (
          <div>
            ...
            />
            <p className="text-[10px] text-muted dark:text-slate-500 mt-1 uppercase tracking-widest">
            ...
            <div className="space-y-2">
              {timesheet.timeLogs.map((log) => (
                <div key={log._id} className="p-4 bg-surface/50 dark:bg-slate-800/50 rounded-xl border border-border-subtle dark:border-slate-700">
                  ...
                  <p className="text-muted dark:text-slate-400 text-sm">{log.description}</p>
                  
                  {/* Time Log Attachments */}
                  {log.attachments?.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-border-subtle dark:border-slate-700/50">
                      <p className="text-[9px] font-black text-muted dark:text-slate-500 uppercase tracking-widest mb-2">
                        LOG ATTACHMENTS
                      </p>
                        ...
                        {log.attachments.map((file, fIdx) => (
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <div className="w-6 h-6 bg-surface/50 dark:bg-slate-700 text-muted dark:text-slate-400 rounded flex items-center justify-center text-[8px] font-bold shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {timesheet.attachments.map((attachment, idx) => (
                ...
                >
                  ...
                  <div className="text-muted dark:text-slate-500 group-hover:text-brand-primary">
                    <Paperclip size={16} />
                  </div>
                </button>
              ))}
        ...
        <div className="pt-6 border-t border-border-subtle dark:border-slate-700">
          <label className="block text-[10px] font-black text-muted dark:text-slate-500 mb-4 uppercase tracking-widest">
            DISCUSSION
          </label>
          ...
          <div className="flex justify-between items-center mb-6 px-2">
            ...
            <p className="text-[10px] text-muted dark:text-slate-500 uppercase tracking-widest">{newComment.length}/200</p>
          </div>
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar-visible">
            {comments.length > 0 ? (
                ...
                <div key={c._id || idx} className="bg-surface dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-border-subtle dark:border-slate-700 flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[10px] text-muted dark:text-slate-500 font-bold whitespace-nowrap ml-2">
              ...
              <p className="text-center text-muted dark:text-slate-500 text-xs italic py-4">No comments yet.</p>
            )}
          </div>
        </div>
      </div>
